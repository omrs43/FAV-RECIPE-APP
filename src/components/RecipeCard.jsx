...
const RecipeCard = ({recipe, bg, badge}) => {
  ...
  const [isFavorite, setIsFavorite] = useState(localStorage.getItem("favorite")?.includes(recipe.label));

  const addRecipeToFavorites = () => {
    let favorites = JSON.parse(localStorage.getItem("favorites")) || [];
    const isRecipeAlreadyInFavorites = favorites.some((fav)=> fav.label == recipe.label.label);
            ...
            <div className="flex gap-2 mt-auto">
              {
                healthLabels.map((label, idx) => (
                  <div key={idx} className={`flex gap-1 ${badge} rounded-md items-center p-1`}>
                <HeartPulse size={16} />
